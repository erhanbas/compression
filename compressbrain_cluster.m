...
opt.inputfolder = sprintf('/tier2/mousebrainmicro/mousebrainmicro/data/%s/Tiling',opt.myshfile(1:end-3))
...
for idx = 1:numfiles
    ...
    name = sprintf('compress_%05d-%s',idx,randString);
    args = sprintf('''./compiled/compiledfiles_conversion/compressbrain_cluster %d %s %s %s %d> output.log''',comp,tiffile,videofolder,outname,delete_file);
